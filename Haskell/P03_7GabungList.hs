...
-- Deskripsi : Menggabungkan dua list menjadi terurut
...
appendData :: [Int] -> Int -> [Int]
-- appendData l x memasukkan x kedalam l secara terurut


-- REALISASI
appendData l x
	| isEmpty l = [x]
	| (head l) > x 	= konso x l									--Basis
	| otherwise		= konso (head l) (appendData (tail l) x)	--REKURENS

gabungList l1 l2
	| isEmpty l1 = l2													--BASIS
	| otherwise	 = gabungList (tail l1) (appendData l2 (head l1))		--REKURENS
	
-- APLIKASI
-- gabungList [1,3,0,4] [-9,-1,4] = [-9,-1,0,1,3,4,4]
-- gabungList [] [2,3,4,5] = [2,3,4,5]
-- gabungList [3,7,2] [] = [2,3,7]
